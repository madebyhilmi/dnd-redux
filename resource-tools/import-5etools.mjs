...
const out = path.resolve('systems/5e2024/resource_instances');
...
for (const m of data.monster ?? []) {
  ...
  await writeFile(path.join(out, `monster_${id}.rpg.json`), `${JSON.stringify({system:'5e2024',resource_id:'monster',stats},null,2)}\n`);
...
if (encounterInput) {
  ...
  for (const item of encounter.items ?? []) {
    ...
    pool.push({ system:'5e2024', resource_id:'monster_combat_template_entry', stats:{ min:value(item.c??1), max:value(item.c??1), weight:value(1), combatant:value({resource_id:'monster',stats:monster.stats}), ...meta() } });
  }
  ...
  await writeFile(path.join(out, `encounter_template_${id}.rpg.json`), `${JSON.stringify({system:'5e2024',resource_id:'encounter_template',stats},null,2)}\n`);
